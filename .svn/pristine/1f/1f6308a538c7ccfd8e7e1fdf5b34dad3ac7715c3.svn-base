Shader "Color Space/YUVtoRGBA" 
{
    Properties 
    {
        _YTex ("Y (RGBA)", 2D) = "black" {}
        _UTex ("U (RGBA)", 2D) = "black" {}
        _VTex ("V (RGBA)", 2D) = "black" {}
        _ATex ("A (RGBA)", 2D) = "black" {}

		_SrcMode ("SrcMode", Float) = 5	//SrcAlpha
		_DstMode ("DstMode", Float) = 10 //OneMinusSrcAlpha
    ...
    SubShader 
    {
        ...
        Pass 
        {
			...
			{		
				...
				a = ( tex2D (_ATex, i.uvY).a - 0.062 ) * 1.164;
				...
				fixed4 rgbVec = fixed4(r, g, b, a * _Opacity);
