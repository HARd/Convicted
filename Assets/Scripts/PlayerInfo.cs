...
public class PlayerInfo : MonoBehaviour,  ISerializeXML 
{
	...
	public string[] GetStringValues()
	{
		string[] str = {health.ToString(), cash.ToString(), energy.ToString(), body.ToString(), charisma.ToString(), mind.ToString(), 
			inmate_rep.ToString(), guard_rep.ToString(), day.ToString(), tunnel.ToString(), consealment.ToString(), charImageID.ToString()};
		return str;
	}
	...
	public void SetStringValues(string[] str)
	{
		...
		charImageID = (int)Convert.ChangeType(str[11], typeof(int));
	}
	...
	{
		GameData.current.AddCharacterCompleted(charImageID);
