...
public class QuestPanelController : MonoBehaviour 
{
	...
	public void OpenQuestLog()
	{
		...
		foreach (Transform child in story.transform) 
		{
			...
			if (!child_story.complete) 
				child_story.OnOpenQuestLog(questControllers[i++]);
		}
	...
	public void RemoveQuest(string char_name)
	{
		...
		for(int j = 0;j<questLog.Count;j++)
		{
			if(questLog[j].char_name == char_name)
			{
	...
	public void CompleteQuest(string char_name, Action quest_action) 
	{
		for(int i = 0;i < questLog.Count;i++) 
		{
			if(questLog[i].char_name == char_name) 
			{
				...
				foreach(Parameter reward in questLog[i].rewards) 
				{
					reward.ChangeStat();
